...
import { AudioOutputSheet, type AudioOutputSheetProps } from './audioOutputControl';
...
function props(overrides: Partial<AudioOutputSheetProps> = {}): AudioOutputSheetProps {
  return {
    // La feuille est OUVERTE par défaut dans ces tests : son `visible` est
    // désormais porté par le parent, et le déclencheur a quitté ce composant
    // pour une ligne du menu « Plus ».
    visible: true,
    onDismiss: jest.fn(),
    mode: 'devices',
    outputs: [],
...
describe('AudioOutputSheet, mode système', () => {
  it('ne monte AUCUNE feuille', async () => {
    // Sur iOS, `getAudioOutputs()` est une constante à deux entrées qui ne sont
    // pas des catégories : il n'y a rien à lire et rien à peupler. Le seul
    // recours est le sélecteur de la plateforme, que `callControlBar.tsx`
    // ouvre — ce composant, lui, ne rend rien.
    //
    ...
    // dans les deux états.
    await render(withPaper(<AudioOutputSheet {...props({ mode: 'system', visible: true })} />));

    expect(screen.queryByTestId('audio-output-device-7')).toBeNull();
    expect(screen.queryByTestId('audio-output-note')).toBeNull();
    expect(screen.queryByTestId('audio-output-sheet-surface')).toBeNull();
  });

  // L'autre borne du mode : sans elle, un composant qui ne rendrait JAMAIS
  // rien passerait le test ci-dessus.
  it('monte la feuille dans les autres modes', async () => {
    await render(withPaper(<AudioOutputSheet {...props({ mode: 'devices', visible: true })} />));

    expect(screen.getByTestId('audio-output-note')).toBeTruthy();
  });
});

describe('AudioOutputSheet, mode catégories', () => {
  ...
  it('envoie la catégorie pressée, pas la première de la liste, et referme', async () => {
    ...
    await render(
      withPaper(
        <AudioOutputSheet
          {...props({ mode: 'menu', outputs: ['bluetooth', 'speaker'], onSelect, onDismiss })}
        />,
      ),
    );
    await waitFor(() => expect(screen.getByTestId('audio-output-option-speaker')).toBeTruthy());
    ...
    // La fermeture n'est plus OBSERVABLE ici : le `visible` appartient au
    // parent depuis que le déclencheur est descendu dans le menu « Plus ». Ce
    // qu'on garde, c'est que la feuille la DEMANDE — l'instruction qui, oubliée,
    // laisserait la feuille ouverte sur un choix déjà fait.
...
describe('AudioOutputSheet, mode appareils', () => {
  it("affiche le nom lu de l'appareil, et la catégorie quand il n'y en a pas", async () => {
    ...
    await render(withPaper(<AudioOutputSheet {...props()} />));
  ...
  it("coche l'appareil CONSTATÉ, pas celui qu'on a demandé", async () => {
    ...
    const view = await render(withPaper(<AudioOutputSheet {...props()} />));
    await waitFor(() => expect(screen.getByTestId('audio-output-device-2')).toBeTruthy());
    expect(screen.queryByTestId('audio-output-check-7')).toBeNull();
    expect(screen.queryByTestId('audio-output-check-2')).toBeNull();

    await view.rerender(withPaper(<AudioOutputSheet {...props({ currentDeviceId: 7 })} />));
  ...
  it("envoie l'appareil pressé, pas le premier de la liste", async () => {
    ...
    await render(withPaper(<AudioOutputSheet {...props({ onSelectDevice })} />));
    await waitFor(() => expect(screen.getByTestId('audio-output-device-2')).toBeTruthy());
  ...
  it('referme la feuille après un choix', async () => {
    ...
    await render(withPaper(<AudioOutputSheet {...props({ onDismiss })} />));
    await waitFor(() => expect(screen.getByTestId('audio-output-device-2')).toBeTruthy());
  ...
  it("change la ligne d'explication après un choix manuel", async () => {
    ...
    await view.rerender(withPaper(<AudioOutputSheet {...props({ manual: true })} />));
  ...
  it("rend la route au système, et referme, quand on revient à l'automatique", async () => {
    ...
    await render(
      withPaper(<AudioOutputSheet {...props({ manual: true, onAutomatic, onDismiss })} />),
    );
    await waitFor(() => expect(screen.getByTestId('audio-output-automatic')).toBeTruthy());
  ...
  it("s'ouvre sur sa seule explication quand la liste est vide", async () => {
    ...
    await render(withPaper(<AudioOutputSheet {...props({ devices: [] })} />));
  ...
  it('force une couleur explicite sur le titre de chaque ligne', async () => {
    ...
    await render(withPaper(<AudioOutputSheet {...props({ manual: true })} />));
    await waitFor(() => expect(screen.getByTestId('audio-output-device-2')).toBeTruthy());
  ...
  it('dessine un vrai glyphe pour la coche, jamais une boîte vide', async () => {
    ...
    // lisible.
    await render(withPaper(<AudioOutputSheet {...props({ currentDeviceId: 7 })} />));
  ...
  it('lave la ligne de l’appareil courant, et elle seule', async () => {
    await render(withPaper(<AudioOutputSheet {...props({ currentDeviceId: 7, manual: true })} />));
