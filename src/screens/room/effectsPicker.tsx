...
/**
 * Le choix de l'effet d'arrière-plan — le CONTENU seul, sans sa feuille.
 *
 * Découpé de `effectsSheet.tsx` parce qu'il est monté à deux endroits qui n'ont
 * pas la même enveloppe : une feuille à lui dans le pré-join, et le menu de la
 * caméra en séance. Le propriétaire a demandé ce rapprochement, et il est
 * juste : l'objectif et l'arrière-plan règlent la même chose, ce que la caméra
 * envoie.
 *
 ...
 */
export function EffectsPicker({ current, onEffectSelect, testID }: Props): React.ReactElement {
    ...
    <>
      <View style={styles.row}>
        <Pressable
          ...
          onPress={() => onEffectSelect({ kind: 'none' })}
        ...
        <Pressable
          accessibilityRole="button"
          onPress={() => onEffectSelect({ kind: 'blur' })}
      ...
      <View style={styles.grid}>
        {backgroundIndexes().map((index) => (
          <Pressable
            accessibilityRole="button"
            key={index}
            onPress={() => onEffectSelect({ index, kind: 'image' })}
            style={styles.cell}
