...
> `contain` y est limitée par la hauteur. **Masquer les deux porte l'image à 787,6 × 443 dp —
...
>   prend sa place dans la bande. Tout le monde reste visible. Un second appui annule. C'est
>   l'épinglage, et les quatre lignes du tableau ci-dessus restent valables.
> - **Appui LONG sur n'importe quelle tuile → PLEIN ÉCRAN** : ni bande, ni barre, ni marqueur.
...
> était juste ; la conclusion ne l'était pas. **Le problème était le geste, pas la fonction.**
> Un appui simple sur une vignette, lui, se découvre tout seul : le partenaire l'a proposé
> spontanément, sans que personne le lui explique, une demi-heure après avoir accepté qu'on
> retire l'épinglage.
>
> Ce qui avait été « assumé » comme une perte acceptable était en réalité un cas d'usage réel
> que rien d'autre ne couvre : **aujourd'hui, dès que quelqu'un partage son écran, tous les
> visages tombent dans la bande et rien ne permet d'en ramener un sur la scène.** Le plein
> écran ne le résout pas — il masque le partage lui-même. Seul l'épinglage le résout.
>
> La leçon, pour la prochaine fois : quand une fonction est rejetée à cause de son affordance,
> corriger l'affordance avant de retirer la fonction.
>
> **Ce qu'aucun test ne prouvera, et qui s'ajoute à la liste :** qu'un appui long se découvre —
> il ne se découvre pas, et c'est admis : il n'ouvre qu'un raccourci vers un état qu'on peut
> aussi atteindre autrement. Que les commandes rappelées en plein écran se retirent au bon
> moment : trop tôt on les rate, trop tard elles gênent. Et qu'un appui simple sur une vignette
> ne se déclenche pas par accident pendant qu'on fait défiler la bande.
